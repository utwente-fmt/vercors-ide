...
export default class VerCorsPathsProvider {
  ...
  public async getPathList(): Promise<VerCorsPath[]> {
    return this.fixPaths(
      await vscode.workspace
        .getConfiguration()
        .get("vercorsplugin.vercorsPath"),
    ) as VerCorsPath[];
  }
  ...
  public async storePathList(vercorsPaths: VerCorsPath[]): Promise<void> {
    ...
    await vscode.workspace
      .getConfiguration()
      .update("vercorsplugin.vercorsPath", this.fixPaths(stored), true);
  }
  ...
  public async deletePath(path: string): Promise<void> {
    ...
    vercorsPaths = vercorsPaths.filter(
      (vercorsPath: VerCorsPath): boolean => vercorsPath.path !== path,
    );
    return this.storePathList(vercorsPaths);
  }

  public async selectVersionFromDialog(
    beforeDetection?: () => void,
    onError?: () => void,
  ): Promise<VerCorsPath | undefined> {
    return vscode.window
      ...
      .then(
        async (
          ...
          if (
            !fs.existsSync(vercorsPath) ||
            !fs.lstatSync(vercorsPath).isFile()
          ) {
          ...
          if (
            vercorsPaths.find(
              (p: VerCorsPath): boolean => p.path === vercorsPath,
            )
          ) {
          ...
          const version: string | undefined =
            await this.detectVersion(vercorsPath);
          if (!version) {
            if (onError) {
              onError();
            }
    ...
    if (paths) {
      for (let i: number = 0; i < paths.length; i++) {
        try {
          ...
          if (
            typeof pathJSON.selected === "boolean" &&
            comparing.eqSet(
              new Set(Object.keys(pathJSON)),
              new Set(["path", "version", "selected"]),
              isEqualPath,
            )
          ) {
