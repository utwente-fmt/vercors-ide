...
export class VerCorsOptions {
  ...
  public static getAllFileOptions(filePath: string): OptionFields {
    return VerCorsOptions.fixOptions(
      vscode.workspace.getConfiguration().get("vercorsplugin.optionsMap", {}),
      filePath,
    ) as OptionFields;
  }

  public static getFlagedOptions(filePath: string): string[] {
    return VerCorsOptions.fixFlagOptions(
      vscode.workspace.getConfiguration().get("vercorsplugin.optionsMap"),
      filePath,
    ) as string[];
  }
  ...
  public static getPinnedOptions(): string[] {
    return this.fixPinnedOptions(
      vscode.workspace.getConfiguration().get("vercorsplugin.optionsMap"),
    ) as string[];
  }

  public static getBackendOption(): backend {
    return this.fixBackendOptions(
      vscode.workspace.getConfiguration().get("vercorsplugin.optionsMap"),
    ) as backend;
  }
  ...
  public static async updateOptions(
    filePath: string,
    vercorsOptions: string[],
    pinnedOptions: string[],
    backendOption: string,
  ): Promise<void> {
    let currentVercorsOptions = (VerCorsOptions.fixOptions(
      vscode.workspace.getConfiguration().get("vercorsplugin.optionsMap", {}),
    ) || {}) as Options;
    currentVercorsOptions[filePath] = {
      flags: vercorsOptions.map((e) => e.trim()),
    };
    ...
    await vscode.workspace
      .getConfiguration()
      .update("vercorsplugin.optionsMap", currentVercorsOptions, true);
  }
  ...
  public static isEqualOptionFields(
    o1: OptionFields,
    o2: OptionFields,
  ): boolean {
    return (
      comparing.compareLists(o1.pinned, o2.pinned) &&
      comparing.compareLists(o1.flags, o2.flags)
    );
  }

  private static fixPinnedOptions(
    options: { [x: string]: string[] } | undefined,
  ): string[] {
    const isStringList =
      options &&
      options["pinned"] &&
      Array.isArray(options["pinned"]) &&
      options["pinned"].every((item) => typeof item === "string");
    return isStringList ? options["pinned"] : [];
  }

  private static fixFlagOptions(
    ...
    if (filePath) {
      ...
      const hasRightValue =
        fileOptionsJSON &&
        Array.isArray(fileOptionsJSON.flags) &&
        comparing.eqSet(
          new Set(Object.keys(fileOptionsJSON)),
          new Set(["flags"]),
        );
      ...
      for (let optionJSON in optionsJSON) {
        if (!(
          ...
          Array.isArray(optionsJSON[optionJSON].flags) &&
          comparing.eqSet(
            new Set(Object.keys(optionsJSON[optionJSON])),
            new Set(["flags"]),
          )
        )) {
          try {
  ...
  private static fixBackendOptions(
    options: { [x: string]: string | number } | undefined,
  ): backend {
    return options && backend[options["backend"] as keyof typeof backend]
      ? backend[options["backend"] as keyof typeof backend]
      : backend.silicon;
  }

  private static fixOptions(
    options: any,
    filePath?: any,
...
export class VerCorsWebViewProvider implements webviewConnector {
  ...
  public async receiveMessage(message: any): Promise<void> {
    switch (message.command) {
      case "updateOptions":
        ...
        await VerCorsOptions.updateOptions(
          filePath!,
          message.options,
          message.pinnedOptions,
  ...
  public async setBackendFileBasePath(): Promise<void> {
    return vscode.window
      ...
      .then(async (fileUri: vscode.Uri[] | undefined): Promise<void> => {
  ...
  private async getHtmlForWebview(_webview: vscode.Webview) {
    ...
    const htmlPath = vscode.Uri.joinPath(
      this._extensionUri,
      "/resources/html/vercorsOptions.html",
    );
  ...
  private async fetchCommandLineOptions() {
    const optionsPath = vscode.Uri.joinPath(
      this._extensionUri,
      "/resources/command-line-options.json",
    );
