...
function activate(context: vscode.ExtensionContext) {
  ...
  const vercorsPath = vscode.workspace
    .getConfiguration()
    .get("vercorsplugin.vercorsPath") as string;
  if (!vercorsPath) {
    vscode.window.showWarningMessage(
      "VerCors binary path is not set. Please set it to run the tool."
    );
  }
  ...
  let disposableSetPath = vscode.commands.registerCommand(
    "extension.setVercorsPath",
    () => {
      setVercorsPath();
    }
  );
...
function executeVercorsCommand() {
  ...
  const vercorsPath =
    path.normalize(
      (vscode.workspace
        .getConfiguration()
        .get("vercorsplugin.vercorsPath") as string) + path.sep
    ) + "vercors";
  ...
  vscode.window
    ...
    .then((path) => {
      if (path !== undefined) {
        vscode.workspace
          .getConfiguration()
          .update("vercorsplugin.vercorsPath", path, true);
      }
    });
}
